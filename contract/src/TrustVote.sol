...
contract TrustVote is ReentrancyGuard {
    ...
    // Mapping
    mapping(uint256 => Election) private electionData; // untuk mapping idElection ke data Election
    mapping(uint256 => Candidate[]) private candidateData; // untuk mapping idElection punya kandidat siapa saja
    mapping(address => uint256[]) private organizerElection; // untuk mapping organizer punya idElection apa saja
    mapping(address => uint256[]) private voterElection; // untuk mapping voter punya idElection apa saja
    mapping(uint256 => address[]) private whiteListedVoter; // untuk mapping idElection punya list whitelist address siapa saja
    ...
    // Getter untuk menampilkan seluruh array idElection yang dipunyai Organizer
    function getOrganizerElection(address addrOrg) public view returns (uint256[] memory) {
        return organizerElection[addrOrg];
    }

    // Getter untuk menampilkan seluruh array idElection yang dipunyai Voters
    function getVoterElection(address addrVoter) public view returns (uint256[] memory) {
        return voterElection[addrVoter];
    }

    // Getter untuk menampilkan whitelisted voter dari suatu idElection 
    function getWhitelistedVoter(uint256 idElection) public view returns (address[] memory) {
        return whiteListedVoter[idElection];
    }
